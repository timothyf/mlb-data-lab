...
import pandas as pd
import pytest
from mlb_data_lab.apis.pybaseball_client import PybaseballClient, process_splits
...
def test_fetch_fangraphs_batter_data_success(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.batting_stats", fake_batting_stats)
...
def test_fetch_fangraphs_batter_data_not_found(monkeypatch, capsys):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.batting_stats", fake_batting_stats_empty)
...
def test_fetch_fangraphs_pitcher_data_success(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.pitching_stats", fake_pitching_stats)
...
def test_fetch_team_batting_stats(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.team_batting", fake_team_batting)
...
def test_fetch_team_pitching_stats(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.team_pitching", fake_team_pitching)
    result = PybaseballClient.fetch_team_pitching_stats("NY", 2020, 2021)
    assert not result.empty
    assert result.iloc[0]['Team'] == "NY"

def test_fetch_team_schedule_and_record(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.schedule_and_record", fake_schedule_and_record)
...
def test_lookup_player(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.playerid_lookup", fake_playerid_lookup)
    result = PybaseballClient.lookup_player("Doe", "John")
    assert not result.empty

def test_lookup_player_by_id(monkeypatch):
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.playerid_reverse_lookup", fake_playerid_reverse_lookup)
...
def test_fetch_batting_splits_leaderboards(monkeypatch):
    ...
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.StatsConfig", lambda: FakeStatsConfig())
    monkeypatch.setattr("mlb_data_lab.apis.pybaseball_client.pyb.get_splits", fake_get_splits)
